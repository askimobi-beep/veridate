...
//     if (!instituteKey) return res.status(400).json({ message: "Target education has no institute" });
...
//     if (!spent) {
//       return res.status(403).json({ message: "No credits left for this institute" });
//     }

//     // 4) add verification to target row (guard against double-verify); refund on conflict
...
//         { _id: verifierId, "verifyCredits.education": { $elemMatch: { instituteKey } } },
//         {
//           $inc: {
//             "verifyCredits.education.$.available": 1,
//             "verifyCredits.education.$.used": -1,
...
//     // 5) reward +1 available to target's same-institute bucket (create bucket if missing)
//     const rewardInc = await User.updateOne(
//       { _id: targetUserId, "verifyCredits.education.instituteKey": instituteKey },
//       { $inc: { "verifyCredits.education.$.available": 1 } }
//     );
...
//     // 6) return fresh, summarized credit buckets (computed totals) + target edu verify counts
...
//       verifier: summarizeEduCredits(verifierUser),
//       target: summarizeEduCredits(targetUser),
//       targetEducation: (refreshedTargetProfile?.education || []).map((r) => ({
//         _id: r._id,
...
//     );
//     if (!exp)
//       return res.status(404).json({ message: "Experience row not found" });
...
//       return res
//         .status(403)
//         .json({ message: "No experience verify credits left" });
...
//       return res
//         .status(409)
//         .json({ message: "Already verified or row changed" });
//     }
...
//     const refreshed = await Profile.findOne({ user: targetUserId })
//       .select("experience")
//       .lean();
//     return res.status(200).json({
